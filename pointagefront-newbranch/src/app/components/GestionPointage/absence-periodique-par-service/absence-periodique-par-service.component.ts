...
export class AbsencePeriodiqueParServiceComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    rechercheAbsenceByService(date1: Date, date2: Date, currentService: Service) {
        ...
        this.tourner=true;
        this.erreur=true;
        this.pointagesService.getAbsencesPeriodiqueParService(this.datepipe.transform(this.date1, 'yyyy-MM-dd'),this.datepipe.transform(this.date2, 'yyyy-MM-dd'),currentService.codeservice).subscribe(data => {
            this.results = data;
